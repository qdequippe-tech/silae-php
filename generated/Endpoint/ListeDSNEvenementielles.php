...
class ListeDSNEvenementielles extends BaseEndpoint implements Endpoint
{
    use EndpointTrait;
     ...
     *             }
     */
    public function __construct(ListeDSNEvenementiellesRequest $request, array $headerParameters = [])
    {
        $this->body = $request;
    ...
    {
        return '/v1/InfosDeclaration/ListeDSNEvenementielles';
    }
    ...
    /**
     * @return ListeDSNEvenementiellesResponse
     *
    ...
    protected function transformResponseBody(ResponseInterface $response, SerializerInterface $serializer, ?string $contentType = null): mixed
    {
        ...
        if (200 === $status) {
            return $serializer->deserialize($body, ListeDSNEvenementiellesResponse::class, 'json');
        }

        if (400 === $status) {
            throw new ListeDSNEvenementiellesBadRequestException($serializer->deserialize($body, ApiErrors::class, 'json'), $response);
        }

        if (401 === $status) {
            throw new ListeDSNEvenementiellesUnauthorizedException($serializer->deserialize($body, ApiErrors::class, 'json'), $response);
        }

        if (500 === $status) {
            throw new ListeDSNEvenementiellesInternalServerErrorException($serializer->deserialize($body, ApiErrors::class, 'json'), $response);
        }
