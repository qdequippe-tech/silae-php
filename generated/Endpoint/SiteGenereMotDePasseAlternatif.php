...
class SiteGenereMotDePasseAlternatif extends BaseEndpoint implements Endpoint
{
    ...
    protected function transformResponseBody(ResponseInterface $response, SerializerInterface $serializer, ?string $contentType = null): mixed
    {
        ...
        if (200 === $status) {
            return $serializer->deserialize($body, SiteGenereMotDePasseAlternatifResponse::class, 'json');
        }

        if (400 === $status) {
            throw new SiteGenereMotDePasseAlternatifBadRequestException($serializer->deserialize($body, ApiErrors::class, 'json'), $response);
        }

        if (401 === $status) {
            throw new SiteGenereMotDePasseAlternatifUnauthorizedException($serializer->deserialize($body, ApiErrors::class, 'json'), $response);
        }
