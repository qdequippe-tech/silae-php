...
class AnalyseProductionPaieBulletinsOriginaux extends BaseEndpoint implements Endpoint
{
    use EndpointTrait;
    ...
    {
        return '/v1/AnalyseProduction/AnalyseProductionPaieBulletinsOriginaux';
    }
    ...
    /**
     * @return AnalyseProductionPaieBulletinsOriginauxResponse
     *
    ...
    protected function transformResponseBody(ResponseInterface $response, SerializerInterface $serializer, ?string $contentType = null): mixed
    {
        ...
        if (200 === $status) {
            return $serializer->deserialize($body, AnalyseProductionPaieBulletinsOriginauxResponse::class, 'json');
        }

        if (400 === $status) {
            throw new AnalyseProductionPaieBulletinsOriginauxBadRequestException($serializer->deserialize($body, ApiErrors::class, 'json'), $response);
        }

        if (401 === $status) {
            throw new AnalyseProductionPaieBulletinsOriginauxUnauthorizedException($serializer->deserialize($body, ApiErrors::class, 'json'), $response);
        }

        if (500 === $status) {
            throw new AnalyseProductionPaieBulletinsOriginauxInternalServerErrorException($serializer->deserialize($body, ApiErrors::class, 'json'), $response);
        }
