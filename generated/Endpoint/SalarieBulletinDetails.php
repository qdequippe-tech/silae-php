...
class SalarieBulletinDetails extends BaseEndpoint implements Endpoint
{
    ...
    /**
     * @return SalarieBulletinDetailsResult
     *
    ...
    protected function transformResponseBody(ResponseInterface $response, SerializerInterface $serializer, ?string $contentType = null)
    {
        ...
        if (200 === $status) {
            return $serializer->deserialize($body, SalarieBulletinDetailsResult::class, 'json');
        }

        if (400 === $status) {
            throw new SalarieBulletinDetailsBadRequestException($serializer->deserialize($body, ApiErrors::class, 'json'), $response);
        }

        if (401 === $status) {
            throw new SalarieBulletinDetailsUnauthorizedException($serializer->deserialize($body, ApiErrors::class, 'json'), $response);
        }

        if (500 === $status) {
            throw new SalarieBulletinDetailsInternalServerErrorException($serializer->deserialize($body, ApiErrors::class, 'json'), $response);
        }
