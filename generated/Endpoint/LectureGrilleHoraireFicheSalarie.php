...
class LectureGrilleHoraireFicheSalarie extends BaseEndpoint implements Endpoint
{
    use EndpointTrait;
     ...
     *             }
     */
    public function __construct(LectureGrilleHoraireFicheSalarieRequest $request, array $headerParameters = [])
    {
        $this->body = $request;
    ...
    {
        return '/v1/SalarieEmplois/LectureGrilleHoraireFicheSalarie';
    }
    ...
    /**
     * @return LectureGrilleHoraireFicheSalarieResponse
     *
    ...
    protected function transformResponseBody(ResponseInterface $response, SerializerInterface $serializer, ?string $contentType = null): mixed
    {
        ...
        if (200 === $status) {
            return $serializer->deserialize($body, LectureGrilleHoraireFicheSalarieResponse::class, 'json');
        }

        if (400 === $status) {
            throw new LectureGrilleHoraireFicheSalarieBadRequestException($serializer->deserialize($body, ApiErrors::class, 'json'), $response);
        }

        if (401 === $status) {
            throw new LectureGrilleHoraireFicheSalarieUnauthorizedException($serializer->deserialize($body, ApiErrors::class, 'json'), $response);
        }

        if (500 === $status) {
            throw new LectureGrilleHoraireFicheSalarieInternalServerErrorException($serializer->deserialize($body, ApiErrors::class, 'json'), $response);
        }
