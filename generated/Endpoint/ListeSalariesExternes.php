...
class ListeSalariesExternes extends BaseEndpoint implements Endpoint
{
    ...
    /**
     * @return ListeSalariesExternesResponse
     *
    ...
    protected function transformResponseBody(ResponseInterface $response, SerializerInterface $serializer, ?string $contentType = null): mixed
    {
        ...
        if (200 === $status) {
            return $serializer->deserialize($body, ListeSalariesExternesResponse::class, 'json');
        }

        if (400 === $status) {
            throw new ListeSalariesExternesBadRequestException($serializer->deserialize($body, ApiErrors::class, 'json'), $response);
        }

        if (401 === $status) {
            throw new ListeSalariesExternesUnauthorizedException($serializer->deserialize($body, ApiErrors::class, 'json'), $response);
        }

        if (500 === $status) {
            throw new ListeSalariesExternesInternalServerErrorException($serializer->deserialize($body, ApiErrors::class, 'json'), $response);
        }
