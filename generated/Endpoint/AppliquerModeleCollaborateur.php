...
class AppliquerModeleCollaborateur extends BaseEndpoint implements Endpoint
{
    use EndpointTrait;
     ...
     *             }
     */
    public function __construct(AppliquerModeleCollaborateurRequest $request, array $headerParameters = [])
    {
        $this->body = $request;
    ...
    {
        return '/v1/AdminCollaborateurs/AppliquerModeleCollaborateur';
    }
    ...
    protected function transformResponseBody(ResponseInterface $response, SerializerInterface $serializer, ?string $contentType = null): mixed
    {
        ...
        if (400 === $status) {
            throw new AppliquerModeleCollaborateurBadRequestException($serializer->deserialize($body, ApiErrors::class, 'json'), $response);
        }

        if (401 === $status) {
            throw new AppliquerModeleCollaborateurUnauthorizedException($serializer->deserialize($body, ApiErrors::class, 'json'), $response);
        }

        if (500 === $status) {
            throw new AppliquerModeleCollaborateurInternalServerErrorException($serializer->deserialize($body, ApiErrors::class, 'json'), $response);
        }
