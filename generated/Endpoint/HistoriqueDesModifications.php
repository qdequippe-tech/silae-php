...
class HistoriqueDesModifications extends BaseEndpoint implements Endpoint
{
    use EndpointTrait;
     ...
     *             }
     */
    public function __construct(HistoriqueDesModificationsRequest $request, array $headerParameters = [])
    {
        $this->body = $request;
    ...
    {
        return '/v1/InfosDossiers/HistoriqueDesModifications';
    }
    ...
    /**
     * @return HistoriqueDesModificationsResponse
     *
    ...
    protected function transformResponseBody(ResponseInterface $response, SerializerInterface $serializer, ?string $contentType = null): mixed
    {
        ...
        if (200 === $status) {
            return $serializer->deserialize($body, HistoriqueDesModificationsResponse::class, 'json');
        }

        if (400 === $status) {
            throw new HistoriqueDesModificationsBadRequestException($serializer->deserialize($body, ApiErrors::class, 'json'), $response);
        }

        if (401 === $status) {
            throw new HistoriqueDesModificationsUnauthorizedException($serializer->deserialize($body, ApiErrors::class, 'json'), $response);
        }

        if (500 === $status) {
            throw new HistoriqueDesModificationsInternalServerErrorException($serializer->deserialize($body, ApiErrors::class, 'json'), $response);
        }
