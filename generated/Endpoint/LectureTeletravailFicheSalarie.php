...
class LectureTeletravailFicheSalarie extends BaseEndpoint implements Endpoint
{
    use EndpointTrait;
     ...
     *             }
     */
    public function __construct(LectureTeletravailFicheSalarieRequest $request, array $headerParameters = [])
    {
        $this->body = $request;
    ...
    {
        return '/v1/SalarieEmplois/LectureTeletravailFicheSalarie';
    }
    ...
    /**
     * @return LectureTeletravailFicheSalarieResponse
     *
    ...
    protected function transformResponseBody(ResponseInterface $response, SerializerInterface $serializer, ?string $contentType = null): mixed
    {
        ...
        if (200 === $status) {
            return $serializer->deserialize($body, LectureTeletravailFicheSalarieResponse::class, 'json');
        }

        if (400 === $status) {
            throw new LectureTeletravailFicheSalarieBadRequestException($serializer->deserialize($body, ApiErrors::class, 'json'), $response);
        }

        if (401 === $status) {
            throw new LectureTeletravailFicheSalarieUnauthorizedException($serializer->deserialize($body, ApiErrors::class, 'json'), $response);
        }

        if (500 === $status) {
            throw new LectureTeletravailFicheSalarieInternalServerErrorException($serializer->deserialize($body, ApiErrors::class, 'json'), $response);
        }
