...
class StatutAnalyseProductionPaieBulletinsCoffresFortsAsynchrone extends BaseEndpoint implements Endpoint
{
    use EndpointTrait;
    ...
    {
        return '/v1/AnalyseProduction/StatutAnalyseProductionPaieBulletinsCoffresFortsAsynchrone';
    }
    ...
    /**
     * @return StatutAnalyseProductionPaieBulletinsCoffresFortsAsynchroneResponse
     *
    ...
    protected function transformResponseBody(ResponseInterface $response, SerializerInterface $serializer, ?string $contentType = null): mixed
    {
        ...
        if (200 === $status) {
            return $serializer->deserialize($body, StatutAnalyseProductionPaieBulletinsCoffresFortsAsynchroneResponse::class, 'json');
        }
        ...
        if (400 === $status) {
            throw new StatutAnalyseProductionPaieBulletinsCoffresFortsAsynchroneBadRequestException($serializer->deserialize($body, ApiErrors::class, 'json'), $response);
        }

        if (401 === $status) {
            throw new StatutAnalyseProductionPaieBulletinsCoffresFortsAsynchroneUnauthorizedException($serializer->deserialize($body, ApiErrors::class, 'json'), $response);
        }

        if (500 === $status) {
            throw new StatutAnalyseProductionPaieBulletinsCoffresFortsAsynchroneInternalServerErrorException($serializer->deserialize($body, ApiErrors::class, 'json'), $response);
        }
