...
class StatutAnalyseProductionPaieRevenusAsynchrone extends BaseEndpoint implements Endpoint
{
    use EndpointTrait;
    ...
    {
        return '/v1/AnalyseProduction/StatutAnalyseProductionPaieRevenusAsynchrone';
    }
    ...
    /**
     * @return StatutAnalyseProductionPaieRevenusAsynchroneResponse
     *
    ...
    protected function transformResponseBody(ResponseInterface $response, SerializerInterface $serializer, ?string $contentType = null): mixed
    {
        ...
        if (200 === $status) {
            return $serializer->deserialize($body, StatutAnalyseProductionPaieRevenusAsynchroneResponse::class, 'json');
        }
        ...
        if (400 === $status) {
            throw new StatutAnalyseProductionPaieRevenusAsynchroneBadRequestException($serializer->deserialize($body, ApiErrors::class, 'json'), $response);
        }

        if (401 === $status) {
            throw new StatutAnalyseProductionPaieRevenusAsynchroneUnauthorizedException($serializer->deserialize($body, ApiErrors::class, 'json'), $response);
        }

        if (500 === $status) {
            throw new StatutAnalyseProductionPaieRevenusAsynchroneInternalServerErrorException($serializer->deserialize($body, ApiErrors::class, 'json'), $response);
        }
