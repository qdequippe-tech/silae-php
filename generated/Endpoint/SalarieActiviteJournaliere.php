...
class SalarieActiviteJournaliere extends BaseEndpoint implements Endpoint
{
    use EndpointTrait;
     ...
     *             }
     */
    public function __construct(SalarieActiviteJournaliereRequest $request, array $headerParameters = [])
    {
        $this->body = $request;
    ...
    {
        return '/v1/ActivitesEtHeures/SalarieActiviteJournaliere';
    }
    ...
    /**
     * @return SalarieActiviteJournaliereResponse
     *
    ...
    protected function transformResponseBody(ResponseInterface $response, SerializerInterface $serializer, ?string $contentType = null): mixed
    {
        ...
        if (200 === $status) {
            return $serializer->deserialize($body, SalarieActiviteJournaliereResponse::class, 'json');
        }

        if (400 === $status) {
            throw new SalarieActiviteJournaliereBadRequestException($serializer->deserialize($body, ApiErrors::class, 'json'), $response);
        }

        if (401 === $status) {
            throw new SalarieActiviteJournaliereUnauthorizedException($serializer->deserialize($body, ApiErrors::class, 'json'), $response);
        }

        if (500 === $status) {
            throw new SalarieActiviteJournaliereInternalServerErrorException($serializer->deserialize($body, ApiErrors::class, 'json'), $response);
        }
