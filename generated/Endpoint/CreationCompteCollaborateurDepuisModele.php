...
class CreationCompteCollaborateurDepuisModele extends BaseEndpoint implements Endpoint
{
    use EndpointTrait;
     ...
     *             }
     */
    public function __construct(CreationCompteCollaborateurDepuisModeleRequest $request, array $headerParameters = [])
    {
        $this->body = $request;
    ...
    {
        return '/v1/AdminCollaborateurs/CreationCompteCollaborateurDepuisModele';
    }
    ...
    protected function transformResponseBody(ResponseInterface $response, SerializerInterface $serializer, ?string $contentType = null): mixed
    {
        ...
        if (400 === $status) {
            throw new CreationCompteCollaborateurDepuisModeleBadRequestException($serializer->deserialize($body, ApiErrors::class, 'json'), $response);
        }

        if (401 === $status) {
            throw new CreationCompteCollaborateurDepuisModeleUnauthorizedException($serializer->deserialize($body, ApiErrors::class, 'json'), $response);
        }

        if (500 === $status) {
            throw new CreationCompteCollaborateurDepuisModeleInternalServerErrorException($serializer->deserialize($body, ApiErrors::class, 'json'), $response);
        }
