...
class SalarieSupprimerAbsence extends BaseEndpoint implements Endpoint
{
     ...
     *             }
     */
    public function __construct(SalarieSupprimerAbsenceRequest $request, array $headerParameters = [])
    {
        $this->body = $request;
    ...
    protected function transformResponseBody(ResponseInterface $response, SerializerInterface $serializer, string $contentType = null)
    {
        ...
        if (400 === $status) {
            throw new SalarieSupprimerAbsenceBadRequestException($serializer->deserialize($body, 'QdequippeTech\\Silae\\Api\\Model\\ApiErrors', 'json'), $response);
        }
        if (401 === $status) {
            throw new SalarieSupprimerAbsenceUnauthorizedException($serializer->deserialize($body, 'QdequippeTech\\Silae\\Api\\Model\\ApiErrors', 'json'), $response);
        }
        if (500 === $status) {
            throw new SalarieSupprimerAbsenceInternalServerErrorException($serializer->deserialize($body, 'QdequippeTech\\Silae\\Api\\Model\\ApiErrors', 'json'), $response);
        }
    }
