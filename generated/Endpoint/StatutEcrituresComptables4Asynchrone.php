...
class StatutEcrituresComptables4Asynchrone extends BaseEndpoint implements Endpoint
{
    use EndpointTrait;
    ...
    {
        return '/v1/EcrituresComptables/StatutEcrituresComptables4Asynchrone';
    }
    ...
    /**
     * @return StatutEcrituresComptables4AsynchroneResponse
     *
    ...
    protected function transformResponseBody(ResponseInterface $response, SerializerInterface $serializer, ?string $contentType = null): mixed
    {
        ...
        if (200 === $status) {
            return $serializer->deserialize($body, StatutEcrituresComptables4AsynchroneResponse::class, 'json');
        }
        ...
        if (400 === $status) {
            throw new StatutEcrituresComptables4AsynchroneBadRequestException($serializer->deserialize($body, ApiErrors::class, 'json'), $response);
        }

        if (401 === $status) {
            throw new StatutEcrituresComptables4AsynchroneUnauthorizedException($serializer->deserialize($body, ApiErrors::class, 'json'), $response);
        }

        if (500 === $status) {
            throw new StatutEcrituresComptables4AsynchroneInternalServerErrorException($serializer->deserialize($body, ApiErrors::class, 'json'), $response);
        }
