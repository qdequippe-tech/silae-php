...
class AnalyseProductionPaieRevenus extends BaseEndpoint implements Endpoint
{
    use EndpointTrait;
    ...
    {
        return '/v1/AnalyseProduction/AnalyseProductionPaieRevenus';
    }
    ...
    /**
     * @return AnalyseProductionPaieRevenusResponse
     *
    ...
    protected function transformResponseBody(ResponseInterface $response, SerializerInterface $serializer, ?string $contentType = null): mixed
    {
        ...
        if (200 === $status) {
            return $serializer->deserialize($body, AnalyseProductionPaieRevenusResponse::class, 'json');
        }

        if (400 === $status) {
            throw new AnalyseProductionPaieRevenusBadRequestException($serializer->deserialize($body, ApiErrors::class, 'json'), $response);
        }

        if (401 === $status) {
            throw new AnalyseProductionPaieRevenusUnauthorizedException($serializer->deserialize($body, ApiErrors::class, 'json'), $response);
        }

        if (500 === $status) {
            throw new AnalyseProductionPaieRevenusInternalServerErrorException($serializer->deserialize($body, ApiErrors::class, 'json'), $response);
        }
