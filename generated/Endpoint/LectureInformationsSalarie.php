...
class LectureInformationsSalarie extends BaseEndpoint implements Endpoint
{
    use EndpointTrait;
    ...
    {
        return '/v1/InfosSalaries/LectureInformationsSalarie';
    }
    ...
    /**
     * @return LectureInformationsSalarieResponse
     *
    ...
    protected function transformResponseBody(ResponseInterface $response, SerializerInterface $serializer, ?string $contentType = null): mixed
    {
        ...
        if (200 === $status) {
            return $serializer->deserialize($body, LectureInformationsSalarieResponse::class, 'json');
        }

        if (400 === $status) {
            throw new LectureInformationsSalarieBadRequestException($serializer->deserialize($body, ApiErrors::class, 'json'), $response);
        }

        if (401 === $status) {
            throw new LectureInformationsSalarieUnauthorizedException($serializer->deserialize($body, ApiErrors::class, 'json'), $response);
        }

        if (500 === $status) {
            throw new LectureInformationsSalarieInternalServerErrorException($serializer->deserialize($body, ApiErrors::class, 'json'), $response);
        }
