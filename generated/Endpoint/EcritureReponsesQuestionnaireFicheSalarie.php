...
class EcritureReponsesQuestionnaireFicheSalarie extends BaseEndpoint implements Endpoint
{
    use EndpointTrait;
     ...
     *             }
     */
    public function __construct(EcritureReponsesQuestionnaireFicheSalarieRequest $request, array $headerParameters = [])
    {
        $this->body = $request;
    ...
    {
        return '/v1/SalarieEmplois/EcritureReponsesQuestionnaireFicheSalarie';
    }
    ...
    protected function transformResponseBody(ResponseInterface $response, SerializerInterface $serializer, ?string $contentType = null): mixed
    {
        ...
        if (400 === $status) {
            throw new EcritureReponsesQuestionnaireFicheSalarieBadRequestException($serializer->deserialize($body, ApiErrors::class, 'json'), $response);
        }

        if (401 === $status) {
            throw new EcritureReponsesQuestionnaireFicheSalarieUnauthorizedException($serializer->deserialize($body, ApiErrors::class, 'json'), $response);
        }

        if (500 === $status) {
            throw new EcritureReponsesQuestionnaireFicheSalarieInternalServerErrorException($serializer->deserialize($body, ApiErrors::class, 'json'), $response);
        }
