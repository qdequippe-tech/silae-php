...
class StatutAnalyseProductionPaieBulletinsOriginauxAsynchrone extends BaseEndpoint implements Endpoint
{
    use EndpointTrait;
    ...
    {
        return '/v1/AnalyseProduction/StatutAnalyseProductionPaieBulletinsOriginauxAsynchrone';
    }
    ...
    /**
     * @return StatutAnalyseProductionPaieBulletinsOriginauxAsynchroneResponse
     *
    ...
    protected function transformResponseBody(ResponseInterface $response, SerializerInterface $serializer, ?string $contentType = null): mixed
    {
        ...
        if (200 === $status) {
            return $serializer->deserialize($body, StatutAnalyseProductionPaieBulletinsOriginauxAsynchroneResponse::class, 'json');
        }
        ...
        if (400 === $status) {
            throw new StatutAnalyseProductionPaieBulletinsOriginauxAsynchroneBadRequestException($serializer->deserialize($body, ApiErrors::class, 'json'), $response);
        }

        if (401 === $status) {
            throw new StatutAnalyseProductionPaieBulletinsOriginauxAsynchroneUnauthorizedException($serializer->deserialize($body, ApiErrors::class, 'json'), $response);
        }

        if (500 === $status) {
            throw new StatutAnalyseProductionPaieBulletinsOriginauxAsynchroneInternalServerErrorException($serializer->deserialize($body, ApiErrors::class, 'json'), $response);
        }
