...
class AnalyseProductionPaieUtilisateursV3 extends BaseEndpoint implements Endpoint
{
    use EndpointTrait;
     ...
     *             }
     */
    public function __construct(AnalyseProductionPaieUtilisateursV3Request $request, array $headerParameters = [])
    {
        $this->body = $request;
    ...
    {
        return '/v1/AnalyseProduction/AnalyseProductionPaieUtilisateursV3';
    }
    ...
    /**
     * @return AnalyseProductionPaieUtilisateursV3Response
     *
    ...
    protected function transformResponseBody(ResponseInterface $response, SerializerInterface $serializer, ?string $contentType = null): mixed
    {
        ...
        if (200 === $status) {
            return $serializer->deserialize($body, AnalyseProductionPaieUtilisateursV3Response::class, 'json');
        }

        if (400 === $status) {
            throw new AnalyseProductionPaieUtilisateursV3BadRequestException($serializer->deserialize($body, ApiErrors::class, 'json'), $response);
        }

        if (401 === $status) {
            throw new AnalyseProductionPaieUtilisateursV3UnauthorizedException($serializer->deserialize($body, ApiErrors::class, 'json'), $response);
        }

        if (500 === $status) {
            throw new AnalyseProductionPaieUtilisateursV3InternalServerErrorException($serializer->deserialize($body, ApiErrors::class, 'json'), $response);
        }
