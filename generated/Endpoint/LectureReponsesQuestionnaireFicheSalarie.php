...
class LectureReponsesQuestionnaireFicheSalarie extends BaseEndpoint implements Endpoint
{
    use EndpointTrait;
    ...
    {
        return '/v1/InfosSalaries/LectureReponsesQuestionnaireFicheSalarie';
    }
    ...
    /**
     * @return LectureReponsesQuestionnaireFicheSalarieResponse
     *
    ...
    protected function transformResponseBody(ResponseInterface $response, SerializerInterface $serializer, ?string $contentType = null): mixed
    {
        ...
        if (200 === $status) {
            return $serializer->deserialize($body, LectureReponsesQuestionnaireFicheSalarieResponse::class, 'json');
        }

        if (400 === $status) {
            throw new LectureReponsesQuestionnaireFicheSalarieBadRequestException($serializer->deserialize($body, ApiErrors::class, 'json'), $response);
        }

        if (401 === $status) {
            throw new LectureReponsesQuestionnaireFicheSalarieUnauthorizedException($serializer->deserialize($body, ApiErrors::class, 'json'), $response);
        }

        if (500 === $status) {
            throw new LectureReponsesQuestionnaireFicheSalarieInternalServerErrorException($serializer->deserialize($body, ApiErrors::class, 'json'), $response);
        }
