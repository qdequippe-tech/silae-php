...
class AnalyseActiviteStatutAccesOptions extends BaseEndpoint implements Endpoint
{
    use EndpointTrait;
     ...
     *             }
     */
    public function __construct(AnalyseActiviteStatutAccesOptionsRequest $request, array $headerParameters = [])
    {
        $this->body = $request;
    ...
    {
        return '/v1/AnalyseProduction/AnalyseActiviteStatutAccesOptions';
    }
    ...
    /**
     * @return AnalyseActiviteStatutAccesOptionsResponse
     *
    ...
    protected function transformResponseBody(ResponseInterface $response, SerializerInterface $serializer, ?string $contentType = null): mixed
    {
        ...
        if (200 === $status) {
            return $serializer->deserialize($body, AnalyseActiviteStatutAccesOptionsResponse::class, 'json');
        }

        if (400 === $status) {
            throw new AnalyseActiviteStatutAccesOptionsBadRequestException($serializer->deserialize($body, ApiErrors::class, 'json'), $response);
        }

        if (401 === $status) {
            throw new AnalyseActiviteStatutAccesOptionsUnauthorizedException($serializer->deserialize($body, ApiErrors::class, 'json'), $response);
        }

        if (500 === $status) {
            throw new AnalyseActiviteStatutAccesOptionsInternalServerErrorException($serializer->deserialize($body, ApiErrors::class, 'json'), $response);
        }
