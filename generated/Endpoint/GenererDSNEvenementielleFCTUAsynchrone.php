...
class GenererDSNEvenementielleFCTUAsynchrone extends BaseEndpoint implements Endpoint
{
    use EndpointTrait;
     ...
     *             }
     */
    public function __construct(GenererDSNEvenementielleFCTURequest $request, array $headerParameters = [])
    {
        $this->body = $request;
    ...
    {
        return '/v1/GenerationDeclaration/GenererDSNEvenementielleFCTUAsynchrone';
    }
    ...
    protected function transformResponseBody(ResponseInterface $response, SerializerInterface $serializer, ?string $contentType = null): mixed
    {
        ...
        if (400 === $status) {
            throw new GenererDSNEvenementielleFCTUAsynchroneBadRequestException($serializer->deserialize($body, ApiErrors::class, 'json'), $response);
        }

        if (401 === $status) {
            throw new GenererDSNEvenementielleFCTUAsynchroneUnauthorizedException($serializer->deserialize($body, ApiErrors::class, 'json'), $response);
        }

        if (500 === $status) {
            throw new GenererDSNEvenementielleFCTUAsynchroneInternalServerErrorException($serializer->deserialize($body, ApiErrors::class, 'json'), $response);
        }
