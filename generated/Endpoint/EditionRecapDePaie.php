...
class EditionRecapDePaie extends BaseEndpoint implements Endpoint
{
     ...
     *             }
     */
    public function __construct(EditionRecapDePaieRequest $request, array $headerParameters = [])
    {
        $this->body = $request;
    ...
    /**
     * @return EditionRecapDePaieResponse
     *
    ...
    protected function transformResponseBody(ResponseInterface $response, SerializerInterface $serializer, ?string $contentType = null): mixed
    {
        ...
        if (200 === $status) {
            return $serializer->deserialize($body, EditionRecapDePaieResponse::class, 'json');
        }

        if (400 === $status) {
            throw new EditionRecapDePaieBadRequestException($serializer->deserialize($body, ApiErrors::class, 'json'), $response);
        }

        if (401 === $status) {
            throw new EditionRecapDePaieUnauthorizedException($serializer->deserialize($body, ApiErrors::class, 'json'), $response);
        }

        if (500 === $status) {
            throw new EditionRecapDePaieInternalServerErrorException($serializer->deserialize($body, ApiErrors::class, 'json'), $response);
        }
