...
class SalarieBulletinStatutEtatAvancement extends BaseEndpoint implements Endpoint
{
    use EndpointTrait;
     ...
     *             }
     */
    public function __construct(SalarieBulletinStatutEtatAvancementRequest $request, array $headerParameters = [])
    {
        $this->body = $request;
    ...
    {
        return '/v1/InfosBulletins/SalarieBulletinStatutEtatAvancement';
    }
    ...
    /**
     * @return SalarieBulletinStatutEtatAvancementResponse
     *
    ...
    protected function transformResponseBody(ResponseInterface $response, SerializerInterface $serializer, ?string $contentType = null): mixed
    {
        ...
        if (200 === $status) {
            return $serializer->deserialize($body, SalarieBulletinStatutEtatAvancementResponse::class, 'json');
        }

        if (400 === $status) {
            throw new SalarieBulletinStatutEtatAvancementBadRequestException($serializer->deserialize($body, ApiErrors::class, 'json'), $response);
        }

        if (401 === $status) {
            throw new SalarieBulletinStatutEtatAvancementUnauthorizedException($serializer->deserialize($body, ApiErrors::class, 'json'), $response);
        }

        if (500 === $status) {
            throw new SalarieBulletinStatutEtatAvancementInternalServerErrorException($serializer->deserialize($body, ApiErrors::class, 'json'), $response);
        }
