...
class AnalyseProductionPaieBulletinsCoffresForts extends BaseEndpoint implements Endpoint
{
    use EndpointTrait;
    ...
    {
        return '/v1/AnalyseProduction/AnalyseProductionPaieBulletinsCoffresForts';
    }
    ...
    /**
     * @return AnalyseProductionPaieBulletinsCoffresFortsResponse
     *
    ...
    protected function transformResponseBody(ResponseInterface $response, SerializerInterface $serializer, ?string $contentType = null): mixed
    {
        ...
        if (200 === $status) {
            return $serializer->deserialize($body, AnalyseProductionPaieBulletinsCoffresFortsResponse::class, 'json');
        }

        if (400 === $status) {
            throw new AnalyseProductionPaieBulletinsCoffresFortsBadRequestException($serializer->deserialize($body, ApiErrors::class, 'json'), $response);
        }

        if (401 === $status) {
            throw new AnalyseProductionPaieBulletinsCoffresFortsUnauthorizedException($serializer->deserialize($body, ApiErrors::class, 'json'), $response);
        }

        if (500 === $status) {
            throw new AnalyseProductionPaieBulletinsCoffresFortsInternalServerErrorException($serializer->deserialize($body, ApiErrors::class, 'json'), $response);
        }
