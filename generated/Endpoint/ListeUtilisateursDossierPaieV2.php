...
class ListeUtilisateursDossierPaieV2 extends BaseEndpoint implements Endpoint
{
    use EndpointTrait;
    ...
    {
        return '/v1/Contacts/ListeUtilisateursDossierPaieV2';
    }
    ...
    /**
     * @return ListeUtilisateursDossierPaieV2Response
     *
    ...
    protected function transformResponseBody(ResponseInterface $response, SerializerInterface $serializer, ?string $contentType = null): mixed
    {
        ...
        if (200 === $status) {
            return $serializer->deserialize($body, ListeUtilisateursDossierPaieV2Response::class, 'json');
        }

        if (400 === $status) {
            throw new ListeUtilisateursDossierPaieV2BadRequestException($serializer->deserialize($body, ApiErrors::class, 'json'), $response);
        }

        if (401 === $status) {
            throw new ListeUtilisateursDossierPaieV2UnauthorizedException($serializer->deserialize($body, ApiErrors::class, 'json'), $response);
        }

        if (500 === $status) {
            throw new ListeUtilisateursDossierPaieV2InternalServerErrorException($serializer->deserialize($body, ApiErrors::class, 'json'), $response);
        }
