...
class ModificationTeletravailFicheSalarie extends BaseEndpoint implements Endpoint
{
    use EndpointTrait;
     ...
     *             }
     */
    public function __construct(ModificationTeletravailFicheSalarieRequest $request, array $headerParameters = [])
    {
        $this->body = $request;
    ...
    {
        return '/v1/SalarieEmplois/ModificationTeletravailFicheSalarie';
    }
    ...
    protected function transformResponseBody(ResponseInterface $response, SerializerInterface $serializer, ?string $contentType = null): mixed
    {
        ...
        if (400 === $status) {
            throw new ModificationTeletravailFicheSalarieBadRequestException($serializer->deserialize($body, ApiErrors::class, 'json'), $response);
        }

        if (401 === $status) {
            throw new ModificationTeletravailFicheSalarieUnauthorizedException($serializer->deserialize($body, ApiErrors::class, 'json'), $response);
        }

        if (500 === $status) {
            throw new ModificationTeletravailFicheSalarieInternalServerErrorException($serializer->deserialize($body, ApiErrors::class, 'json'), $response);
        }
