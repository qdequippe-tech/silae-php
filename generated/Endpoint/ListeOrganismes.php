...
class ListeOrganismes extends BaseEndpoint implements Endpoint
{
    use EndpointTrait;
     ...
     *             }
     */
    public function __construct(ListeOrganismesRequest $request, array $headerParameters = [])
    {
        $this->body = $request;
    ...
    {
        return '/v1/Organisme/ListeOrganismes';
    }
    ...
    /**
     * @return ListeOrganismesResponse
     *
    ...
    protected function transformResponseBody(ResponseInterface $response, SerializerInterface $serializer, ?string $contentType = null): mixed
    {
        ...
        if (200 === $status) {
            return $serializer->deserialize($body, ListeOrganismesResponse::class, 'json');
        }

        if (400 === $status) {
            throw new ListeOrganismesBadRequestException($serializer->deserialize($body, ApiErrors::class, 'json'), $response);
        }

        if (401 === $status) {
            throw new ListeOrganismesUnauthorizedException($serializer->deserialize($body, ApiErrors::class, 'json'), $response);
        }

        if (500 === $status) {
            throw new ListeOrganismesInternalServerErrorException($serializer->deserialize($body, ApiErrors::class, 'json'), $response);
        }
