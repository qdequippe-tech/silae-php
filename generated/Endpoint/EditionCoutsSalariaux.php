...
class EditionCoutsSalariaux extends BaseEndpoint implements Endpoint
{
    ...
    /**
     * @return EditionCoutsSalariauxResponse
     *
    ...
    protected function transformResponseBody(ResponseInterface $response, SerializerInterface $serializer, ?string $contentType = null): mixed
    {
        ...
        if (200 === $status) {
            return $serializer->deserialize($body, EditionCoutsSalariauxResponse::class, 'json');
        }

        if (400 === $status) {
            throw new EditionCoutsSalariauxBadRequestException($serializer->deserialize($body, ApiErrors::class, 'json'), $response);
        }

        if (401 === $status) {
            throw new EditionCoutsSalariauxUnauthorizedException($serializer->deserialize($body, ApiErrors::class, 'json'), $response);
        }

        if (500 === $status) {
            throw new EditionCoutsSalariauxInternalServerErrorException($serializer->deserialize($body, ApiErrors::class, 'json'), $response);
        }
