...
class EcrituresComptablesPaiementSalaires extends BaseEndpoint implements Endpoint
{
    use EndpointTrait;
     ...
     *             }
     */
    public function __construct(EcrituresComptablesPaiementSalairesRequest $request, array $headerParameters = [])
    {
        $this->body = $request;
    ...
    {
        return '/v1/EcrituresComptables/EcrituresComptablesPaiementSalaires';
    }
    ...
    protected function transformResponseBody(ResponseInterface $response, SerializerInterface $serializer, ?string $contentType = null): mixed
    {
        ...
        if (400 === $status) {
            throw new EcrituresComptablesPaiementSalairesBadRequestException($serializer->deserialize($body, ApiErrors::class, 'json'), $response);
        }

        if (401 === $status) {
            throw new EcrituresComptablesPaiementSalairesUnauthorizedException($serializer->deserialize($body, ApiErrors::class, 'json'), $response);
        }

        if (500 === $status) {
            throw new EcrituresComptablesPaiementSalairesInternalServerErrorException($serializer->deserialize($body, ApiErrors::class, 'json'), $response);
        }
