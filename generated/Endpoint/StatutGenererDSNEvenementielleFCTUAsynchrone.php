...
class StatutGenererDSNEvenementielleFCTUAsynchrone extends BaseEndpoint implements Endpoint
{
    use EndpointTrait;
    ...
    {
        return '/v1/GenerationDeclaration/StatutGenererDSNEvenementielleFCTUAsynchrone';
    }
    ...
    /**
     * @return StatutGenererDSNEvenementielleFCTUResponse
     *
    ...
    protected function transformResponseBody(ResponseInterface $response, SerializerInterface $serializer, ?string $contentType = null): mixed
    {
        ...
        if (200 === $status) {
            return $serializer->deserialize($body, StatutGenererDSNEvenementielleFCTUResponse::class, 'json');
        }
        ...
        if (400 === $status) {
            throw new StatutGenererDSNEvenementielleFCTUAsynchroneBadRequestException($serializer->deserialize($body, ApiErrors::class, 'json'), $response);
        }

        if (401 === $status) {
            throw new StatutGenererDSNEvenementielleFCTUAsynchroneUnauthorizedException($serializer->deserialize($body, ApiErrors::class, 'json'), $response);
        }

        if (500 === $status) {
            throw new StatutGenererDSNEvenementielleFCTUAsynchroneInternalServerErrorException($serializer->deserialize($body, ApiErrors::class, 'json'), $response);
        }
