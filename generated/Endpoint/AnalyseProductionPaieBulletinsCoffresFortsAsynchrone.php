...
class AnalyseProductionPaieBulletinsCoffresFortsAsynchrone extends BaseEndpoint implements Endpoint
{
    use EndpointTrait;
    ...
    {
        return '/v1/AnalyseProduction/AnalyseProductionPaieBulletinsCoffresFortsAsynchrone';
    }
    ...
    protected function transformResponseBody(ResponseInterface $response, SerializerInterface $serializer, ?string $contentType = null): mixed
    {
        ...
        if (400 === $status) {
            throw new AnalyseProductionPaieBulletinsCoffresFortsAsynchroneBadRequestException($serializer->deserialize($body, ApiErrors::class, 'json'), $response);
        }

        if (401 === $status) {
            throw new AnalyseProductionPaieBulletinsCoffresFortsAsynchroneUnauthorizedException($serializer->deserialize($body, ApiErrors::class, 'json'), $response);
        }

        if (500 === $status) {
            throw new AnalyseProductionPaieBulletinsCoffresFortsAsynchroneInternalServerErrorException($serializer->deserialize($body, ApiErrors::class, 'json'), $response);
        }
