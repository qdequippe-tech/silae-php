...
class AnalyseProductionPaieEffectifs extends BaseEndpoint implements Endpoint
{
    use EndpointTrait;
     ...
     *             }
     */
    public function __construct(AnalyseProductionPaieEffectifsRequest $request, array $headerParameters = [])
    {
        $this->body = $request;
    ...
    {
        return '/v1/AnalyseProduction/AnalyseProductionPaieEffectifs';
    }
    ...
    /**
     * @return AnalyseProductionPaieEffectifsResponse
     *
    ...
    protected function transformResponseBody(ResponseInterface $response, SerializerInterface $serializer, ?string $contentType = null): mixed
    {
        ...
        if (200 === $status) {
            return $serializer->deserialize($body, AnalyseProductionPaieEffectifsResponse::class, 'json');
        }

        if (400 === $status) {
            throw new AnalyseProductionPaieEffectifsBadRequestException($serializer->deserialize($body, ApiErrors::class, 'json'), $response);
        }

        if (401 === $status) {
            throw new AnalyseProductionPaieEffectifsUnauthorizedException($serializer->deserialize($body, ApiErrors::class, 'json'), $response);
        }

        if (500 === $status) {
            throw new AnalyseProductionPaieEffectifsInternalServerErrorException($serializer->deserialize($body, ApiErrors::class, 'json'), $response);
        }
