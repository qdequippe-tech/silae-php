...
class StatutSalariesBulletinsAsynchrone extends BaseEndpoint implements Endpoint
{
    ...
    /**
     * @return StatutSalariesBulletinsResponse
     *
    ...
    protected function transformResponseBody(ResponseInterface $response, SerializerInterface $serializer, ?string $contentType = null): mixed
    {
        ...
        if (200 === $status) {
            return $serializer->deserialize($body, StatutSalariesBulletinsResponse::class, 'json');
        }
        ...
        if (400 === $status) {
            throw new StatutSalariesBulletinsAsynchroneBadRequestException($serializer->deserialize($body, ApiErrors::class, 'json'), $response);
        }

        if (401 === $status) {
            throw new StatutSalariesBulletinsAsynchroneUnauthorizedException($serializer->deserialize($body, ApiErrors::class, 'json'), $response);
        }

        if (500 === $status) {
            throw new StatutSalariesBulletinsAsynchroneInternalServerErrorException($serializer->deserialize($body, ApiErrors::class, 'json'), $response);
        }
