...
class EcrituresComptables4Asynchrone extends BaseEndpoint implements Endpoint
{
    use EndpointTrait;
     ...
     *             }
     */
    public function __construct(EcrituresComptables4Request $request, array $headerParameters = [])
    {
        $this->body = $request;
    ...
    {
        return '/v1/EcrituresComptables/EcrituresComptables4Asynchrone';
    }
    ...
    protected function transformResponseBody(ResponseInterface $response, SerializerInterface $serializer, ?string $contentType = null): mixed
    {
        ...
        if (400 === $status) {
            throw new EcrituresComptables4AsynchroneBadRequestException($serializer->deserialize($body, ApiErrors::class, 'json'), $response);
        }

        if (401 === $status) {
            throw new EcrituresComptables4AsynchroneUnauthorizedException($serializer->deserialize($body, ApiErrors::class, 'json'), $response);
        }

        if (500 === $status) {
            throw new EcrituresComptables4AsynchroneInternalServerErrorException($serializer->deserialize($body, ApiErrors::class, 'json'), $response);
        }
