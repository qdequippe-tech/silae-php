...
class EditionRecapDePaieDetailleParSalarie extends BaseEndpoint implements Endpoint
{
    use EndpointTrait;
     ...
     *             }
     */
    public function __construct(EditionRecapDePaieDetailleParSalarieRequest $request, array $headerParameters = [])
    {
        $this->body = $request;
    ...
    {
        return '/v1/EditionEtatsPaie/EditionRecapDePaieDetailleParSalarie';
    }
    ...
    /**
     * @return EditionRecapDePaieDetailleParSalarieResponse
     *
    ...
    protected function transformResponseBody(ResponseInterface $response, SerializerInterface $serializer, ?string $contentType = null): mixed
    {
        ...
        if (200 === $status) {
            return $serializer->deserialize($body, EditionRecapDePaieDetailleParSalarieResponse::class, 'json');
        }

        if (400 === $status) {
            throw new EditionRecapDePaieDetailleParSalarieBadRequestException($serializer->deserialize($body, ApiErrors::class, 'json'), $response);
        }

        if (401 === $status) {
            throw new EditionRecapDePaieDetailleParSalarieUnauthorizedException($serializer->deserialize($body, ApiErrors::class, 'json'), $response);
        }

        if (500 === $status) {
            throw new EditionRecapDePaieDetailleParSalarieInternalServerErrorException($serializer->deserialize($body, ApiErrors::class, 'json'), $response);
        }
