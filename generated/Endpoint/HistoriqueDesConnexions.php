...
class HistoriqueDesConnexions extends BaseEndpoint implements Endpoint
{
    use EndpointTrait;
     ...
     *             }
     */
    public function __construct(HistoriqueDesConnexionsRequest $request, array $headerParameters = [])
    {
        $this->body = $request;
    ...
    {
        return '/v1/InfosDossiers/HistoriqueDesConnexions';
    }
    ...
    /**
     * @return HistoriqueDesConnexionsResponse
     *
    ...
    protected function transformResponseBody(ResponseInterface $response, SerializerInterface $serializer, ?string $contentType = null): mixed
    {
        ...
        if (200 === $status) {
            return $serializer->deserialize($body, HistoriqueDesConnexionsResponse::class, 'json');
        }

        if (400 === $status) {
            throw new HistoriqueDesConnexionsBadRequestException($serializer->deserialize($body, ApiErrors::class, 'json'), $response);
        }

        if (401 === $status) {
            throw new HistoriqueDesConnexionsUnauthorizedException($serializer->deserialize($body, ApiErrors::class, 'json'), $response);
        }

        if (500 === $status) {
            throw new HistoriqueDesConnexionsInternalServerErrorException($serializer->deserialize($body, ApiErrors::class, 'json'), $response);
        }
