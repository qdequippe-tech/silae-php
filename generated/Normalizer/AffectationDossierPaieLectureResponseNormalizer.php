...
class AffectationDossierPaieLectureResponseNormalizer implements DenormalizerInterface, NormalizerInterface, DenormalizerAwareInterface, NormalizerAwareInterface
{
    ...
    {
        return \is_object($data) && AffectationDossierPaieLectureResponse::class === $data::class;
    }
    ...
    {
        $dataArray = [];
        if ($data->isInitialized('affectationDossier') && null !== $data->getAffectationDossier()) {
            $values = [];
            foreach ($data->getAffectationDossier() as $value) {
                $values[] = $value;
            }

            $dataArray['affectationDossier'] = $values;
        }
