...
class CreationUtilisateurClientPaieRequestNormalizer implements DenormalizerInterface, NormalizerInterface, DenormalizerAwareInterface, NormalizerAwareInterface
{
    ...
    {
        return \is_object($data) && CreationUtilisateurClientPaieRequest::class === $data::class;
    }
    ...
    {
        $dataArray = [];
        if ($data->isInitialized('utilisateurClient') && null !== $data->getUtilisateurClient()) {
            $dataArray['utilisateurClient'] = $this->normalizer->normalize($data->getUtilisateurClient(), 'json', $context);
        }
