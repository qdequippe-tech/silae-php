...
class EtatDeclarationsResponseNormalizer implements DenormalizerInterface, NormalizerInterface, DenormalizerAwareInterface, NormalizerAwareInterface
{
    ...
    {
        return EtatDeclarationsResponse::class === $type;
    }

    public function supportsNormalization(mixed $data, ?string $format = null, array $context = []): bool
    {
        return \is_object($data) && EtatDeclarationsResponse::class === $data::class;
    }
    ...
    public function denormalize(mixed $data, string $type, ?string $format = null, array $context = []): mixed
    {
        ...
        if (\array_key_exists('etatDeclarations', $data) && null !== $data['etatDeclarations']) {
            ...
            foreach ($data['etatDeclarations'] as $value) {
                $values[] = $this->denormalizer->denormalize($value, EtatDeclaration::class, 'json', $context);
            }
    ...
    {
        $dataArray = [];
        if ($data->isInitialized('etatDeclarations') && null !== $data->getEtatDeclarations()) {
            $values = [];
            foreach ($data->getEtatDeclarations() as $value) {
                $values[] = $this->normalizer->normalize($value, 'json', $context);
            }

            $dataArray['etatDeclarations'] = $values;
        }
    ...
    {
        return [EtatDeclarationsResponse::class => false];
    }
}
