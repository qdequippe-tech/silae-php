...
class ExtraListeManifestationsResponseNormalizer implements DenormalizerInterface, NormalizerInterface, DenormalizerAwareInterface, NormalizerAwareInterface
{
    ...
    {
        return \is_object($data) && ExtraListeManifestationsResponse::class === $data::class;
    }
    ...
    {
        $dataArray = [];
        if ($data->isInitialized('manifestations') && null !== $data->getManifestations()) {
            $values = [];
            foreach ($data->getManifestations() as $value) {
                $values[] = $this->normalizer->normalize($value, 'json', $context);
            }

            $dataArray['manifestations'] = $values;
        }
